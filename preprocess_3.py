import cv2
import numpy as np
import os
from concurrent.futures import ThreadPoolExecutor

def detect_skin_simple(image):
    """Phát hiện vùng da bằng ngưỡng màu YCrCb (Nhanh hơn K-Means)."""
    ycrcb = cv2.cvtColor(image, cv2.COLOR_BGR2YCrCb)
    Cr, Cb = cv2.split(ycrcb)[1:3]

    # Áp dụng threshold trực tiếp lên kênh Cr và Cb để xác định vùng da
    skin_mask = cv2.inRange(ycrcb, (0, 133, 77), (255, 173, 127))

    # Giảm nhiễu bằng phép toán hình thái học đơn giản hơn
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3))
    skin_mask = cv2.morphologyEx(skin_mask, cv2.MORPH_OPEN, kernel)

    skin_ratio = np.sum(skin_mask > 0) / (skin_mask.shape[0] * skin_mask.shape[1])
    return skin_mask, skin_ratio

def check_image_quality(image):
    """Kiểm tra độ sắc nét của ảnh bằng độ phương sai Laplacian."""
    gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    lap_var = cv2.Laplacian(gray, cv2.CV_64F).var()
    
    # Đặt ngưỡng động dựa trên kích thước ảnh
    threshold = max(100, 0.00005 * (gray.shape[0] * gray.shape[1]))
    return lap_var > threshold

def zoom_to_roi(image, skin_mask, zoom_factor=1.3):
    """Phóng to vùng da được phát hiện để tập trung vào khu vực quan trọng."""
    contours, _ = cv2.findContours(skin_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    if contours:
        x, y, w, h = cv2.boundingRect(max(contours, key=cv2.contourArea))

        # Mở rộng vùng ROI (giảm số phép toán)
        padding = max(w, h) // 4
        x1, x2 = max(0, x - padding), min(image.shape[1], x + w + padding)
        y1, y2 = max(0, y - padding), min(image.shape[0], y + h + padding)

        # Cắt và phóng to ảnh
        cropped = image[y1:y2, x1:x2]
        zoomed = cv2.resize(cropped, (image.shape[1], image.shape[0]), interpolation=cv2.INTER_LINEAR)
        return zoomed
    return image

def preprocess_image(image_path, target_size=224, resize_factor=0.5):
    """Tiền xử lý ảnh: giảm kích thước, phát hiện da, kiểm tra chất lượng và chuẩn hóa."""
    image = cv2.imread(image_path)
    if image is None:
        raise ValueError(f"Không thể đọc ảnh từ {image_path}.")

    # Giảm kích thước ảnh trước khi xử lý
    small_image = cv2.resize(image, (0, 0), fx=resize_factor, fy=resize_factor, interpolation=cv2.INTER_AREA)

    # Phát hiện vùng da
    skin_mask, skin_ratio = detect_skin_simple(small_image)

    # Kiểm tra chất lượng ảnh
    is_clear = check_image_quality(small_image)

    # Nếu ảnh không rõ hoặc vùng da nhỏ, phóng to vùng da quan trọng
    if not is_clear or skin_ratio < 0.3:
        image = zoom_to_roi(image, skin_mask)

    # Resize ảnh về kích thước mong muốn và chuẩn hóa
    resized_image = cv2.resize(image, (target_size, target_size), interpolation=cv2.INTER_AREA)
    normalized_image = resized_image.astype(np.float32) / 255.0

    return normalized_image, skin_mask
